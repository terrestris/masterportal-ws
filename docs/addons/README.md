...
Mittels eines Addons lässt sich die Funktionalität des Masterportals beliebig erweitern, ohne dass der **Core** verändert werden muss. Es lassen sich eigenständige `Tools` und `GfiThemes` entwickeln, die zu Beginn der Laufzeit importiert werden und fortan wie eigenständige Module funktionieren. Desweiteren gibt es die Typen `javascript` und `searchInterface`: Mit ersterem Typ lässt sich beliebiger Code hinzufügen, der beispielsweise als Action aufgerufen werden kann - aber bewusst kein Tool sein soll. Mit dem Typ `searchInterface` lassen sich benutzerdefinierte Suchbackends hinzufügen.
